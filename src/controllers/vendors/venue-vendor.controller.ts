...
const getVenueVendorDashboard = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    const veneueVendorData = await venueVendorService.getDashboardData(
      req.user?.id
    );
...
const getVenueVendorService = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    const venueVendorData = await venueVendorService.getVenue({
...
const changeBookingStatus = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    const bookingData = await venueService.changeBookingStatus(
      bookingId,
      status,
      req?.user?.id
    );
...
const generateAdvancePayment = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    const bookingData = await venueService.generateAdvancePayment(
      bookingId,
      advancePayment
    );
...
const getAvailabilityInfo = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    const availabilityData = await venueService.getAvailabilityInfo(
      req?.user?.id
    );
...
const generateFullPayment = asyncHandler(
  async (req: CustomRequest, res: Response): Promise<void> => {
    ...
    const bookingData = await venueService.generateFullPayment(
      bookingId,
      fullPaymentCharges
    );
