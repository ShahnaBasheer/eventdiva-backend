...
import { generateNewToken, verifyToken, isVendorDocument } from '../utils/helperFunctions';
...
const authMiddleware = asyncHandler(async (req: CustomRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        if((user as IcustomerDocument | IVendorDocument).isBlocked){
            let tokenKey = (role === UserRole.Customer)?process.env.CUSTOMER_REFRESH:process.env.VENDOR_REFRESH;
        ...
        if(user && role === UserRole.Vendor && isVendorDocument(user)){
            if(user.vendorType === VendorType.EventPlanner){
              const eventPlanner = await eventPlannerService.getEventPlanner({ vendorId: user.id });
              ...
              if (eventPlanner) {
                ...
                const venueVendor = await venueVendorService.getVenue({ vendorId: user.id });
                if (venueVendor) {
        ...
        if (error instanceof jwt.TokenExpiredError) {
            ...
            if(!refreshToken && (role === UserRole.Admin || role === UserRole.Vendor)){
            ...
            try {
                ...
                res.clearCookie(tokenKey!);
                if(error instanceof ForbiddenError || error instanceof UnauthorizedError) throw error;
...
const isUser = asyncHandler(async (req: CustomRequest, res: Response, next: NextFunction): Promise<void> => {
    if (req?.user) {
        return next();
    }
...
const authenticateSocket = async (socket: CustomSocket, next: (err?: any) => void) => {
    let role: string | undefined;
